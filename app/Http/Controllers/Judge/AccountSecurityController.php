...
class AccountSecurityController extends Controller
{
    ...
    {
        $validated = $request->validateWithBag('confirmTwoFactorAuthentication', ['code' => ['required', 'string', 'size:6']]);
    ...
    public function regenerateRecoveryCodes(Request $request, GenerateNewRecoveryCodes $generate): RedirectResponse
    {
        ...
        if (! $request->user()->hasEnabledTwoFactorAuthentication()) {
            throw ValidationException::withMessages(['two_factor' => 'Confirma primero la autenticación en dos pasos.'])
    ...
    {
        $request->validateWithBag('twoFactorAuthentication', ['current_password' => ['required', 'string', 'current_password']]);
    }
}
