...
<div class="row g-4 mt-2">
    <div class="col-lg-6">
        <form method="POST" action="{{ route('user-profile-information.update') }}" class="card ff-card p-4">
            ...
            <input class="form-control mb-3 @if($errors->getBag('updateProfileInformation')->has('name')) is-invalid @endif" id="name" name="name" value="{{ old('name', auth()->user()->name) }}" required autocomplete="name">
            @foreach($errors->getBag('updateProfileInformation')->get('name') as $message)
                <div class="invalid-feedback d-block mb-3">{{ $message }}</div>
            @endforeach
            <label class="form-label" for="email">Correo electrónico</label>
            <input class="form-control mb-3 @if($errors->getBag('updateProfileInformation')->has('email')) is-invalid @endif" id="email" name="email" type="email" value="{{ old('email', auth()->user()->email) }}" required autocomplete="email">
            @foreach($errors->getBag('updateProfileInformation')->get('email') as $message)
                <div class="invalid-feedback d-block mb-3">{{ $message }}</div>
            @endforeach
        ...
        <div class="card ff-card p-4">
            ...
            @if($twoFactorConfirmed)
                ...
                <form method="POST" action="{{ route('panel.account.two-factor.recovery-codes') }}" class="mt-4">
                    ...
                    <input class="form-control mb-2 @if($errors->getBag('twoFactorAuthentication')->has('current_password')) is-invalid @endif" id="two_factor_regenerate_password" name="current_password" type="password" required autocomplete="current-password">
                ...
                <form method="POST" action="{{ route('panel.account.two-factor.disable') }}">
                    ...
                    <input class="form-control mb-2 @if($errors->getBag('twoFactorAuthentication')->has('current_password')) is-invalid @endif" id="two_factor_disable_password" name="current_password" type="password" required autocomplete="current-password">
            ...
            @elseif($twoFactorPending)
                ...
                <form method="POST" action="{{ route('panel.account.two-factor.confirm') }}" class="mt-4" novalidate>
                    ...
                    <input class="form-control mb-2 @if($errors->getBag('confirmTwoFactorAuthentication')->has('code')) is-invalid @endif" id="two_factor_code" name="code" inputmode="numeric" autocomplete="one-time-code" maxlength="6" pattern="[0-9]{6}" required>
                    @foreach($errors->getBag('confirmTwoFactorAuthentication')->get('code') as $message)
            ...
            @else
                <form method="POST" action="{{ route('panel.account.two-factor.enable') }}">
                    ...
                    <input class="form-control mb-2 @if($errors->getBag('twoFactorAuthentication')->has('current_password')) is-invalid @endif" id="two_factor_enable_password" name="current_password" type="password" required autocomplete="current-password">
                    @foreach($errors->getBag('twoFactorAuthentication')->get('current_password') as $message)
                        <div class="invalid-feedback d-block mb-2">{{ $message }}</div>
                    @endforeach
