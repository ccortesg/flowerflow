...
> **Hallazgo vigente — 2026-08-18:** `flowerflow_testing`/`flowerflow_testing_user` sigue siendo el único ambiente autorizado para pruebas destructivas. M5 acreditó 18/18 migraciones y suite verde bajo el contrato final `4 primary + 2 substitute`, todos ilimitados. La base primaria `flowerflow` no se tocó. No corregir, consultar destructivamente ni migrar otros recursos por inferencia; M6 debe volver a demostrar el guard exacto.
