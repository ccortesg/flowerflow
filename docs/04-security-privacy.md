...
> **Controles ADR-0016 — 2026-08-26:** ambos alcances conservan rol exacto admin, permisos acumulativos, contraseña reciente, ownership, flag, job cifrado y archivo privado. La exportación vigente reduce datos a propuesta/categoría, juez, estado, revisión y evidencia de calificación; excluye participante, contacto, proyecto, reaperturas y motivos. `current_revision_id` inconsistente aborta el libro, todo contenido es texto literal y ni filtros GET ni exportación modifican negocio.

> **Controles ADR-0015 — 2026-08-25:** asignar a `pending_setup` no omite `verified` ni `judge.active`; el juez sigue sin poder entrar, descargar, declarar conflicto o evaluar. La exportación exige rol exacto `admin`, tres permisos acumulativos, contraseña reciente, flag, ownership y archivo vigente. Proyecto/participante/categoría salen sólo del snapshot; no existe fallback vivo. Las celdas son texto literal y se excluyen motivo de reapertura, contacto, archivos, rutas, tokens y metadata. Auditoría y job conservan sólo IDs, conteos, estados y códigos redactados.

> **Controles M8A — 2026-08-25:** proyecto y exportaciones sólo consumen la proyección allowlist del paquete ciego inmutable; nunca el snapshot crudo, identidad, contacto, admisibilidad, residencia, notas o nombres originales. Cada GET revalida juez exacto, ownership, assignment activo, ausencia de conflicto, evaluación iniciada, versión, paquete activo, schema y hash. PDF deshabilita remoto/PHP/JavaScript; XLSX fuerza texto literal. Los temporales viven fuera de `public`, salen con `private, no-store`/`nosniff`, se eliminan tras la respuesta y tienen throttle por juez/asignación/formato. Autosave conserva CSRF, allowlist, locks, BCMath y 409 sin overwrite; no usa storage del navegador ni telemetría.
...
> **Controles M6A — 2026-08-24:** el enlace inicial combina firma temporal, token opaco hasheado, fingerprint del correo, CSRF en POST, rate limit, slot único y locks; GET es puro y sólo el POST válido verifica. La selección administrativa rechaza atómicamente IDs duplicados, ajenos o jueces no operativos; no confía en IDs de rúbrica/package/evaluation del navegador. La notificación de asignación omite identidad, título, contenido y archivos. Auditoría sólo admite IDs técnicos, conteos, flags, versiones, transiciones y reason codes. La excepción jurídica de cero mínimos bloquea release/producción.

> **Adenda de exportación de contactos — 2026-08-24, local/test:** `Exportar Contactos` reutiliza `export submissions`, contraseña reciente, job cifrado post-commit, disk privado, ownership y expiración de 24 horas. Incluye únicamente propuestas `submitted` y toma nombre, correo, teléfono, título y descripción del snapshot inmutable; nunca consulta perfil o contenido vivo como fallback. Las celdas son texto literal y la auditoría sólo registra tipo, estados, conteos y códigos redactados, sin PII ni contenido.
