...
> **Decisiones ADR-0016 — 2026-08-26:** `Q-ADR16-001 RESOLVED LOCAL`: “última revisión” es `current_revision_id` y una fila por evaluación/juez; drift aborta. `Q-ADR16-002 RESOLVED LOCAL`: el alcance vigente se añade sin sustituir historial y el POST legado conserva `all_revisions_v1`. `Q-ADR16-003 RESOLVED LOCAL`: ambos XLSX son globales e independientes de filtros. No se resolvieron ni autorizaron consolidación, ranking, ganadores, producción o el mínimo jurídico.

> **Decisiones ADR-0015 — 2026-08-25:** `Q-ADR15-001 RESOLVED LOCAL`: `pending_setup` es asignable administrativamente, pero no operativo y sin replay de correo. `Q-ADR15-002 RESOLVED LOCAL`: ID de propuesta significa ULID público y comparte parámetro `folio`. `Q-ADR15-003 RESOLVED LOCAL`: export dedicado, privado, owner-only, tres hojas y snapshot como única fuente de identidad/proyecto. Permanece abierto y bloqueante únicamente el frente jurídico ya registrado; activación y capacidad productiva continúan `POR_CONFIRMAR`.

> **Decisiones de asignación simultánea — 2026-08-25:** `Q-BULK-001 RESOLVED LOCAL`: límite síncrono 20 y éxito parcial por transacción de propuesta. `Q-BULK-002 RESOLVED LOCAL`: no hay tabla batch; intención cifrada, lock y auditoría terminal. `Q-BULK-003 RESOLVED LOCAL`: un delivery consolidado opcional, sin PII/contenido. `Q-BULK-004 RESOLVED LOCAL / RELEASE POR_CONFIRMAR`: 20 propuestas con 10 MiB cada una ejecutaron en 2.589 s localmente; timeout/capacidad productivos no fueron autorizados ni verificados. El mínimo jurídico sigue abierto y bloqueante.
