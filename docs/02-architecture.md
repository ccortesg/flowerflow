...
> **Arquitectura ADR-0016 — 2026-08-26:** Asignaciones y Evaluaciones consumen el mismo `SubmissionReferenceFilter`; filtros GET anidados no mutan y conservan query string. `EvaluationExportScope` separa `current_revisions_v1` de `all_revisions_v1` sin migración. El job existente selecciona un camino de writer: el histórico queda intacto y el vigente transmite dos hojas, valida `current_revision_id` contra el agregado append-only y falla cerrado ante drift.

> **Arquitectura ADR-0015 — 2026-08-25:** `AdministrativeJudgeEligibility` separa capacidad administrativa de asignación y acceso efectivo del juez; los tres flujos de asignación consumen la misma autoridad y los deliveries se omiten antes de crearse para `pending_setup`. `SubmissionReferenceFilter` agrupa folio/ULID y escapa comodines. `evaluation_exports` es un agregado privado separado, procesado por un job cifrado post-commit en `database/exports`; `EvaluationWorkbookWriter` transmite tres hojas desde snapshots/revisiones persistidas y falla cerrado ante una fuente inválida.

> **Arquitectura M8A — 2026-08-25:** el detalle del juez se separa en cuatro GET de lectura (inicio, proyecto, evaluación y confirmación) manteniendo el POST/PATCH M6/M7 como únicas fronteras de mutación. `JudgeEvaluationWorkspace` proyecta estado y completitud; `BlindReviewProjectResolver` entrega únicamente el paquete ciego fijado y valida schema/hash. `intent=autosave` reutiliza `SaveEvaluationDraft`, lock optimista y cálculo BCMath. PDF/XLSX son síncronos, privados y acotados; no añaden migración, cola, worker o estado persistente. ADR-0014 documenta las dependencias y la frontera de seguridad.
