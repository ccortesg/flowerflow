...
| R75 | Diseño presentado como implementación | Bajo documental controlado | M1–M7 están conformes local/test; M8–M10 no están implementados | Mantener estado por milestone y no atribuir evidencia local a producción |
...
| R78 | Admin modifica puntajes y puede aparentar suplantación del juez | **Residual controlado local/test por M7** | Sólo revisiones reabiertas; `submitted_by_user_id` real, juez sujeto estable, motivo cifrado, contraseña/confirmación, auditoría y fuente inmutable | Revisión operativa previa al release; la notificación corresponde a M8 |
