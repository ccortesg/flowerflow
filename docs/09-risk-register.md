...
|---|---|---:|---|---|
| R57 | Runtime local primario no representa el SHA actual | Alto operativo local | El baseline de `flowerflow` registró migraciones pendientes y no es autoridad; M1/M2 se validaron en `flowerflow_testing` con 14/14 y guard exacto | Release candidate únicamente en ambiente autorizado y sintético; no migrar por inferencia |
...
| R68 | Snapshot enviado filtra identidad al juez | **Crítico / diseño mitigado** | Se aprobó paquete ciego estructural; el snapshot crudo sigue conteniendo identidad y nunca puede servirse directamente | Implementar allowlist versionada en M5 y pruebas canary; riesgo semántico separado en R77 |
| R69 | Rúbrica o cálculo ambiguo altera puntajes | **Diseño resuelto / implementación pendiente** | Criterios, pesos, escala, paso, precisión, `HALF_UP`, media y faltantes están en ADR-0008 | Implementar versión inmutable/cálculo servidor en M3/M6 y probar vectores límite |
...
| R71 | Conflicto/reasignación inconsistente por concurrencia | **Alto para M4; decisión cerrada** | Catálogo/autoridad definidos y quinto sustituto aprobado con capacidad diez; el flujo aún no existe | Locks, estado terminal, reemplazo referenciado, conteo activo y rechazo de la undécima sustitución |
...
| R75 | Diseño presentado como implementación | Bajo documental controlado | M1/M2 están marcados 20 % funcional y separados de M3–M10 no implementados; preparación 90 % no se confunde con código | Mantener estado por milestone y no atribuir M1/M2 locales a producción |
| R76 | Capacidad/cobertura/sustitución incompatibles (`P2B-BLOCK-001`) | **Resuelto por propietario 2026-08-18** | Cuatro principales sin límite fijo cubren todas las propuestas elegibles; quinto sustituto exclusivo con capacidad diez. Perfil M2 lo expresa como `primary=NULL` y `substitute=10` | M4 debe implementar exactamente el contrato, sin carga inicial al sustituto ni límite artificial a principales |
| R77 | Ceguera simple permite autoidentificación dentro del contenido | Alto aceptado por propietario | Se elimina PII estructurada, pero texto, imagen, enlace o anexo pueden revelar identidad y el owner acepta no bloquear | UI honesta, allowlist/metadatos limpios, tests; no prometer anonimización total ni registrar contenido en logs |
...
| R80 | Más de diez sustituciones simultáneas | Alto operativo aceptado | El quinto juez sólo admite diez reasignaciones activas; no se autorizó sobreasignación ni un segundo sustituto | M4 falla cerrado ante la undécima, alerta a admin y exige una decisión operativa nueva antes de ampliar capacidad |
