...
> **ADR-0016 local/test — 2026-08-26:** no añade migración, ruta, permiso, flag, dependencia o proceso. El worker existente acepta `current_revisions_v1` y `all_revisions_v1`; solicitudes antiguas sin alcance conservan historial. La UI recomienda vigente, pero ambos archivos son globales. Antes de activar debe repetirse smoke de ambos tipos, validación de worker/disk/retención y UAT de filtros. Rollback: `FLOWERFLOW_EVALUATION_EXPORT_ENABLED=false`; nunca borrar filas o archivos vigentes.

> **ADR-0015 local/test — 2026-08-25:** añade `FLOWERFLOW_EVALUATION_EXPORT_ENABLED=false`, permiso `export evaluations` y migración 25. El worker existente `--queue=high,exports,default,low` procesa el XLSX privado; no se agrega proceso ni dependencia. Antes de una activación separada deben verificarse migración, RBAC, disk privado, worker `exports`, scheduler de purga y smoke con datos sintéticos. Rollback: apagar el flag; no borrar filas, archivos vigentes o auditoría. Los jueces `pending_setup` pueden recibir assignments, pero no deliveries ni acceso. Producción no autorizada.
