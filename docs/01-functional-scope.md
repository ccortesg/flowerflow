...
> **Contrato vigente de preparación y exportación — 2026-08-25, local/test:** un `admin` puede asignar individualmente, en lote o como reemplazo a un juez coherente en `pending_setup`; la asignación no concede acceso y no genera correo hasta que la cuenta sea operativa, ni existe replay. Propuestas y Admisibilidad buscan por folio o ULID público con el mismo campo. Evaluaciones añade un XLSX privado, asíncrono y exclusivo del solicitante con todas las revisiones, criterios y reaperturas, uniendo identidad del snapshot inmutable sólo dentro del archivo autorizado. Flag default-off, permiso exclusivo y expiración de 24 horas. Producción continúa no autorizada.

> **Asignación simultánea — 2026-08-25, local/test:** un asistente administrativo permite elegir un juez y hasta veinte propuestas, revisar sin mutar y ejecutar admisión, paquete ciego y asignación con una transacción independiente por propuesta. Los expedientes faltantes siguen requiriendo backfill; los ya admitidos no se sobrescriben. El correo al juez es opcional y consolidado mediante el outbox. Flag default-off, sin tabla batch, worker nuevo, despliegue o producción. Persiste el bloqueo jurídico de cobertura mínima.

> **Contrato vigente M8 — 2026-08-25, local/test:** conflicto declarado/resuelto, envío/reenvío y reapertura generan comunicaciones transaccionales mediante el outbox existente; el cierre genera un digest único por juez con conteos. Los destinatarios son exactos, los eventos son ID-only post-commit, la revalidación cancela estados obsoletos y no existe fallback legacy. No hay replay histórico ni recordatorios programados 20/22 de agosto. M9–M10, consolidación, ranking, resultados, producción y SMTP real continúan fuera. `NO-GO RELEASE/PRODUCTION — OWNER_OVERRIDE / LEGAL_RECONCILIATION_REQUIRED`.
...
> **Sustitución parcial aprobada, 2026-07-15 (histórica):** Fase 01 implementó sitio público, auth, perfil, borradores/envío, archivos y panel mínimo. La adenda Fase 02A autorizó después admisibilidad. El estado vigente posterior añade M1–M8 locales; consolidación, ganadores y resultados continúan sin implementar.
