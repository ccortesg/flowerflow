...
Este ExecPlan es un documento vivo y se rige por `AGENTS.md` y `.agent/PLANS.md`. La autorización proviene de la solicitud expresa del propietario del 2026-08-17. El trabajo es exclusivamente local/test: no autoriza stage, commit, push, acceso a AWS, cambios en servicios compartidos ni despliegue.
...
- [x] 2026-08-17 MST — El propietario sustituyó dos veces la Mecánica v1.1 durante el pre-release. Se observaron `0101aa79…` (tres/tres), `1b537e42…` (tabla de cuatro con dos referencias aún obsoletas) y el PDF definitivo `11c399ca…`, que confirma cuatro categorías y máximo cuatro propuestas en pp. 2–3. Sólo queda `POR_CONFIRMAR P1` la superposición de accesibilidad entre Movilidad con Flow y Hermosillo sin Barreras.
- [!] 2026-08-17 MST — El PDF v1.0 de Mecánica fue modificado históricamente en el commit `dca0bfd` sin cambiar su nombre de versión: el archivo actual tiene SHA-256 `3bcf31…`, pero el registro/seeder histórico conserva `42bd5e…`. No se reescribe evidencia previa; se documenta como incidente de integridad.
...
- [!] 2026-08-17 MST — El 503 de recepción cerrada usa la página Laravel predeterminada y genera dos reportes CSP por estilo inline. Es P2/`PROPOSAL_NEEDED` porque corregirla cambia UX; no se implementa en este alcance.
...
- `PROPOSAL_NEEDED`: la existencia de v1.1 no decide por sí sola si usuarios con aceptación v1.0 deben reaceptar; se determinará sólo con evidencia explícita o aprobación posterior.
- `POR_CONFIRMAR P1`: confirmar si “accesibilidad” pertenece deliberadamente tanto a Movilidad con Flow como a Hermosillo sin Barreras. Las cantidades cuatro/cuatro están resueltas y no requieren rollback.
- `PENDING crítico de evidencia`: recuperar el binario original de Mecánica v1.0 asociado al hash `42bd5e…` sin reemplazar el artefacto físico actual `3bcf31…` ni reescribir aceptaciones.
